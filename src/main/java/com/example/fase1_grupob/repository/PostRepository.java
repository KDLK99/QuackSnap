...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = """
            SELECT * FROM post p NATURAL JOIN (SELECT DISTINCT cp.posts_id FROM category c NATURAL JOIN category_posts cp WHERE cp.category_id=:Category_id) as ccpi WHERE p.id=posts_id ORDER BY CASE WHEN :orderby = 'likes' THEN p.likes
                                                                                                                                                                                                       WHEN :orderby = 'comments' THEN p.counter
                                                                                                                                                                                                       END DESC""", nativeQuery = true)
    List<Post> findPostsByCategoryIDOrdered(int Category_id, String orderby);
